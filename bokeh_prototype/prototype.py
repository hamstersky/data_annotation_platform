...
def get_data_range(frame_nr):
    ...
    global data
    subset = data[
        (frame_nr >= data["frame_in"]) & (frame_nr <= data["frame_out"] + 400)
    ]
    subset_segments = segments[
        (frame_nr >= segments["frame_in"])
        & (frame_nr <= segments["frame_out"] + 400)
        & segments["correct"]
        == True
    ]
    ...
    l.data_source.data = {
        ...
        **{
            ...
            "line_width": [2.0] * N,
            "line_dash": ["solid"]
            * N,  # 'solid', 'dashed', 'dotted', 'dotdash', 'dashdot'
        },
    }
...
img_plot = p.image_rgba(
    source=img_source, image="image", x=0, y=0, dw=cap_w, dh=cap_h, level="image"
)
...
curdoc().add_root(
    column(slider, forward, p, table, connect, wrong, par, segments_table)
)
